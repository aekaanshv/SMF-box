...
double pred_fn(model S, double const *x)
{
  ...
  VecD y_vec(1);
  x1.push_back(x_temp);
  predictor(S,1,x1,y_vec);
  y = y_vec[0];
//  std::cout<<" Fn worked and the val it gave is : "<<y<<std::endl;
  return y;
}


//double * cm_optimize(int dim, model S, double(*pFun)(model, double const *), VecD xlow, VecD xupp, int number_of_restarts, double increment_factor_for_population_size);
...
//
  ...
  model S;
  S.num_pts = 24;
  int num_pts = S.num_pts;
  S.dim = dim;
  VecD theta(dim,4),y0(num_pts),temp(dim);
  MatD x0;
  for(int i = 0; i<num_pts;i++)
  {
	...
	for(int j=0;j<dim;j++)
	{
		temp[j] = 0.1*(2*i + 0.02*j + 1);
		y0[i] += 2*(temp[j]*temp[j] - 2*temp[j]);
	}
//      y0[i] = 2*((2*i+1)*(2*i+1-2)); 
//	y0[i] = 2*((2*i+1)*(2*i+1-2) + (2*i+2)*(2*i+2-2) );
  ...
  S.theta = theta;
  // Optimize function
  x = cm_optimize(dim, S, pred_fn, nbrestarts, incpopsize);
...
bool is_feasible(VecD xlow, VecD xupp, double const *x)
{
	int dim = xlow.size();
 ...
 */
double * cm_optimize(int dim, model S, double(*pFun)(model S,double const *), VecD xlow, VecD xupp, int nrestarts, double incpopsize, VecD inguess, VecD in_stddev, double TolX)
{
  ...
  for (irun = 0; irun < nrestarts+1; ++irun)
  {
    ...
    double xstart[dim];
    for(int i=0; i<dim; i++) xstart[i] = inguess[i];
    double stddev[dim];
    for(int i=0; i<dim; i++) stddev[i] = in_stddev[i];
...
//    std::cout << evo.sayHello() << std::endl;
   ...
   int stop_thresh = dim*2E2;
   int i_stop = 0;
    while((!evo.testForTermination())&&(i_stop < stop_thresh))
    { 
      ...
      /* You may resample the solution i until it lies within the
        ...
        */
         while (!is_feasible(xlow,xupp,pop[i])) 
         {	    
		    evo.reSampleSingle(i);

		    i_stop = i_stop + 1;
		    if(i_stop%500 == 0)
			std::cout<<"STUCK IN IS_FEASIBLE -.- wITH I_STOP :"<<i_stop<<"  \n";
	 }
        
        fitvals[i] = (*pFun)(S,pop[i]); 
      }
    ...
    if(i_stop >= stop_thresh)
	std::cout<<" OY CODE STALLED : \n \n MANUAL BREAK IN RESAMPLING FOR CMA-ES \n";
...
//    evo.writeToFile(CMAES<double>::WKResume, "resumeevo2.dat");
    ...
    if((fmean = (*pFun)(S,evo.getPtr(CMAES<double>::XMean))) < fbestever)
    {
      fbestever = fmean;
      xbestever = evo.getInto(CMAES<double>::XMean, xbestever);
    }

    std::cout<<"\nBest solution value = "<<fbestever<<"\n";
